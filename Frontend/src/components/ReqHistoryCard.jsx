...
const ReqHistoryCard = ({
  ...
  const handleConfirmDelete = async () => {
    console.log(" Attempting to delete pickup:", {
      requestId,
    ...
    try {
      ...
      if (res.data.success) {
        ...
        toast.error(res.data.message || "Failed to delete pickup");
      }
    } catch (error) {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to delete pickup";
  ...
  const handleModify = () => {
    if (onUpdate) {
      // Pass the correct pickup object with _id
      onUpdate({
        _id: requestId,
        address,
        items,
      ...
      <div className="flex place-self-center-safe w-full flex-col bg-transparent border-2 border-black rounded-[20px] relative text-start p-3 mb-3">
        ...
        <div className="flex gap-1.5 mt-3">
          <button
            ...
            title={
              !isPending
                ? "Only pending pickups can be modified"
                : "Modify pickup"
            }
          >
            Modify
        ...
        <div
          className={`${getStatusColor(
            status
          )} rounded-xl px-2 py-1 absolute top-2.5 right-2 text-xs font-medium`}
        >
